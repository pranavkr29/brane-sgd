...
class Functions:
    ...
    def __init__(self, N4):
        ...
        self.SUSY_coeff_1 = 0.01
        self.SUSY_coeff_2 = 0.05
        self.SUSY_coeff_3 = 0.02
    ...
    def brane_potential(
        ...
        D4_sqnorm = (
            tf.norm(
                tf.linalg.matmul(Z41, tf.math.conj(Z41))
                + tf.linalg.matmul(Z42, tf.math.conj(Z42))
                + tf.linalg.matmul(Z43, tf.math.conj(Z43))
                - tf.linalg.matmul(tf.math.conj(Z14), Z14)
                - tf.linalg.matmul(tf.math.conj(Z24), Z24)
                - tf.linalg.matmul(tf.math.conj(Z34), Z34)
                + self.commutator(phi1, tf.math.conj(phi1))
                + self.commutator(phi2, tf.math.conj(phi2))
                + self.commutator(phi3, tf.math.conj(phi3))
        ...
        return (
            ...
            + self.SUSY_coeff_1 * tf.norm(phi1) ** 2
            + self.SUSY_coeff_2 * tf.norm(phi2) ** 2
            + self.SUSY_coeff_3 * tf.norm(phi3) ** 2
        )
    ...
    def B_invariant(
        ...
        """Receives combined variables to produce a list of"""
        B = []
...
class Solver:
    ...
    def init_run(self, n_steps, learning_rate=1e-3):
        ...
        for _ in tqdm(range(self.functions.num_cycles)):
            ...
            if loss_val < 1e-9 or gradient_norm < 1e-6 or tf.math.is_nan(loss_val):
                break
...
def main(N4, num_runs, n_steps, learning_rate):
    ...
    for i in results:
        ...
        if (
            i[-1] < 1e-6 or i[0] < 1e-6 or (i[-1] < 1 and i[0] < 1)
        ):  # Keeping low derivative or low loss solutions
    ...
    if len(solutions_list) > 0:
        ...
        s1, s2, s3 = (
            functions.SUSY_coeff_1,
            functions.SUSY_coeff_2,
            functions.SUSY_coeff_3,
        )
        t = time.time()
        fname = f"U1markers_N4={N4}_s1={s1}_s2={s2}_s3={s3}_t={t}"
        fname_sol = f"U1fullsols_N4={N4}_s1={s1}_s2={s2}_s3={s3}_t={t}"
